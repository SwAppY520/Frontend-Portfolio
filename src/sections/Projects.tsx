import uiuxAgencyLandingPage from "@/assets/images/UIUX Agency Landing Page.png";
import clothingLandingPage from "@/assets/images/Clothing Landing Page.png";
import frontendPortfolioImage from "@/assets/images/frontend-portfolio.png";
import Image from "next/image";
import CheckCircleIcon from "@/assets/icons/check-circle.svg";
import ArrowUpRight from "@/assets/icons/arrow-up-right.svg";
import { Card } from "@/components/Card";


const portfolioProjects = [
  {
    company: "Lazarev",
    year: "2025",
    title: "UI/UX Agency Landing Page",
    results: [
      { title: "Built with HTML, CSS, and JavaScript." },
      { title: "Smooth animations using GSAP." },
      { title: "Designed a user-friendly interface." },
    ],
    link: "https://ui-ux-agency-landing-page.vercel.app/",
    image: uiuxAgencyLandingPage,
  },
  {
    company: "Rosyz",
    year: "2024",
    title: "Clothing Landing Page",
    results: [
      { title: "Built with HTML, CSS, and JavaScript." },
      { title: "Smooth animations." },
      { title: "JavaScript for dynamic interactions." },
    ],
    link: "https://clothing-website-fawn-eta.vercel.app/",
    image: clothingLandingPage,
  },
  {
    company: "Portfolio",
    year: "2025",
    title: "Frontend Portfolio",
    results: [
      { title: "Build with ReactJS TailwindCSS Framer Motion" },
      { title: "Smooth animation using TailwindCSS" },
      { title: "Designed a Responsive Layout" },
    ],
    link: "https://swapnilkalambe.vercel.app/",
    image: frontendPortfolioImage,
  },
];

export const ProjectsSection = () => {
  return (
    <section className="pb-16 lg:py-24" id="Projects">
      <div className="container">
        <div className="flex justify-center">
          <p className="uppercase font-semibold tracking-widest bg-gradient-to-r from-emerald-300 to-sky-400 text-transparent bg-clip-text">
            Personal Projects
          </p>
        </div>

        <h2 className="font-poppins md:text-5xl text-3xl text-center mt-6">
          Featured Projects
        </h2>
        <p className="text-center text-white/60 mt-4 md:text-lg lg:text-xl max-w-md mx-auto">
          See how I transform concepts into engaging digital experience.
        </p>
        <div className="flex flex-col mt-10 gap-20 md:mt-20">
          {portfolioProjects.map((project, projectIndex) => (
            <Card
              key={project.title}
              className="px-8 pt-8 md:py-12 md:px-10 lg:pt-16 lg:px-20 sticky" style={{
                top: `calc(64px + ${projectIndex * 40}px`
              }}
              >

              <div className="lg:grid lg:grid-cols-2 lg:gap-16">
                <div className="lg:pb-16">
                  <div className="bg-gradient-to-r from-emerald-300 to-sky-400 inline-flex gap-2 font-bold uppercase tracking-widest text-sm text-transparent bg-clip-text">
                    <span>{project.company}</span>
                    <span>&bull;</span>
                    <span>{project.year}</span>
                  </div>

                  <h3 className="font-poppins text-2xl mt-2 md:mt-5 md:text-4xl">
                    {project.title}
                  </h3>
                  <hr className="border-t-2 border-white/5 mt-4 md:mt-5" />
                  <ul className="flex flex-col gap-4 mt-4 md:mt-5">
                    {project.results.map((result) => (
                      <li
                        key={result.title}
                        className="flex gap-2 text-sm md:text-base  text-white/50"
                      >
                        <CheckCircleIcon className="size-5 md:size-6" />
                        <span>{result.title}</span>
                      </li>
                    ))}
                  </ul>
                  <a href={project.link}>
                    <button className="bg-white text-gray-950 h-12 w-full md:w-auto px-6 rounded-xl font-semibold inline-flex items-center justify-center gap-2 mt-8">
                      <span>Visit Live Site</span>
                      <ArrowUpRight className="size-4" />
                    </button>
                  </a>
                </div>
                <div className="relative">
                  <Image
                    src={project.image}
                    alt={project.title}
                    className="mt-8 -mb-4 md:-mb-12 lg:mt-12 lg:absolute lg:h-full lg:w-auto lg:max-w-none rounded-t-2xl"
                  />
                </div>
              </div>
            </Card>
          ))}
        </div>
      </div>
    </section>
  );
};
